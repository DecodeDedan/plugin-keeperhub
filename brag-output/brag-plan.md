...
|---|---|---|
| ElizaOS session, the transfer | Chrome DevTools screencast, frame by frame with arrival timestamps, driven through the real UI | 79.5s, 3660 frames |
| ElizaOS session, a refusal | same method, a second session | 31.5s, 682 frames |
...
|---|---|---|---|---|
| 0 | 0.0-5.0 | 5.0s | An agent that is right 99% of the time drains a treasury on its hundredth transfer | composition |
| 1 | 5.0-11.0 | 6.0s | Logo, wordmark, "One agent, one transfer, recorded start to finish." | composition |
| 2 | 11.0-15.7 | 4.7s | The request typed into the ElizaOS chat | footage, 1.5x |
| 3 | 15.7-18.9 | 3.2s | The local model composing, labelled 8x | footage, 8x |
| 4 | 18.9-24.9 | 6.0s | KEEPERHUB_SIMULATE returns the exact plan | footage, real time |
| 5 | 24.9-33.4 | 8.5s | "Nothing has moved." held alone | composition |
| 6 | 33.4-35.4 | 2.0s | `confirm` typed by a human | footage, real time |
| 7 | 35.4-39.5 | 4.1s | KEEPERHUB_CONFIRM replaying the stored payload, labelled 8x | footage, 8x |
| 8 | 39.5-45.4 | 5.9s | The receipt: execution id, transaction hash | footage, real time |
| 9 | 45.4-53.4 | 8.0s | Task id, derived key and stored args from a dry run at capture time | docs/run.json |
| 10 | 53.4-65.4 | 12.0s | Etherscan: success, block, internal transfer of 0.001 ETH | screenshot, slow drift |
| 10b | 65.4-68.1 | 2.7s | A second request, one the model gets wrong | footage, real time |
| 10c | 68.1-71.1 | 3.0s | The model composing again, labelled 8x | footage, 8x |
| 10d | 71.1-75.4 | 4.3s | The refusal: the address does not parse, nothing queued | still from that session |
| 11 | 75.4-86.1 | 10.7s | The landing page running the same gate | footage, 1.2x |
| 12 | 86.1-94.5 | 8.4s | Where each value on that page comes from | docs/run.json |
| 13 | 94.5-105.0 | 10.5s | npm install, "The agent proposes. You approve.", ElizaOS x KeeperHub | composition |

### The refusal beat

The second request asked for 0.05 SOL to a Solana address on chain 101. What the film shows is
not the Solana refusal that was aimed for: the local model returned a `to_address` that parses
as neither an EVM nor a base58 address, and the plugin rejected it at the boundary, before any
call. That is a truer failure than the scripted one, so it was kept. Two takes produced the
same result, which is itself the point: a 14B model asked to transcribe a 32 character address
gets it wrong, and nothing reaches the chain when it does.

The screencast stops emitting frames while the page is idle, so the last 4.3 seconds hold a
still captured from that same session rather than moving footage. Nothing else in the film is
a still except the explorer page.
...
| Scene | Caption |
|---|---|
| 0 | An agent that is right 99% of the time drains a treasury on its hundredth transfer / onchain, there is no undo |
...
| 10b | Now a request the model gets wrong. |
| 10c | It answers with an address that is not an address. |
| 10d | Rejected before anything was sent. Nothing queued, nothing signed. |
...
## Audio
- No voiceover. A music bed at 0.40 carries the whole 105 seconds: mean -23.0 dB, peak -5.4 dB.
